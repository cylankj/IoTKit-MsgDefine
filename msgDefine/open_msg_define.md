...
|---|---|---|
|id| int | 开放功能唯一标识 |
|time| int64| 单位毫秒。查询时间点 |

说明：
* time统一以服务器时间戳为准。
* 如果asc为true，正向查询，返回time之后的数据。
* 如果asc为false，逆向查询，返回time之前的数据。
...
* time 由服务器提供，客户端收到该消息后，更新该消息的缓存时间戳。
...
|---|---|---|
|id| int | 开放功能唯一标识 |
|time| int64| 单位毫秒。删除时间点 |

* time 如果time为真，删除匹配条件的单条记录。如果time为-1, 则删除该消息的全部消息。
...
* time统一以服务器时间戳为准。
* 如果asc为true，升序查询，返回time之后的数据。后续查询使用响应的dataList最大时间。
* 如果asc为false，降序查询，返回time之前的数据。后续查询使用响应的dataList最小时间。
* 如果time置为0, 默认使用服务器当前时间。
* 服务器当前响应最大条数为100.
* 服务器响应条数为0，所有数据已查完。
...
## 2.12 消息定义 - 客户端操作配置后，服务器推送给设备端；设备端事件触发后，服务器推送给APP端
